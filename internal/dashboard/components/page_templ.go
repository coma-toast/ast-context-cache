...
func PageTemplate(projects []Project, health HealthInfo) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 1, "<!doctype html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>AST Context Cache - Dashboard</title><link rel=\"preconnect\" href=\"https://fonts.googleapis.com\"><link rel=\"preconnect\" href=\"https://fonts.gstatic.com\" crossorigin><link href=\"https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600&family=Inter:wght@400;500;600;700&display=swap\" rel=\"stylesheet\"><link rel=\"stylesheet\" href=\"/static/styles.css\"><script src=\"/static/htmx.min.js\"></script><script defer src=\"/static/alpine.min.js\"></script></head><body><div class=\"app-shell\" x-data=\"dashboard()\"><aside class=\"sidebar\" aria-label=\"Dashboard navigation\"><div class=\"sidebar-brand\"><span class=\"sidebar-logo\" aria-hidden=\"true\">&#x1F9E0;</span><div class=\"sidebar-brand-text\"><span class=\"sidebar-title\">AST Context Cache</span> <span class=\"sidebar-subtitle\">Operator dashboard</span></div></div><nav class=\"sidebar-nav\"><button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'overview' }\" @click=\"setTab('overview')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">◈</span> <span>Overview</span></button> <button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'memory' }\" @click=\"setTab('memory')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">◎</span> <span>Memory</span></button> <button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'activity' }\" @click=\"setTab('activity')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">↗</span> <span>Activity</span></button> <button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'analytics' }\" @click=\"setTab('analytics')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">▤</span> <span>Analytics</span></button> <button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'recent' }\" @click=\"setTab('recent')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">☰</span> <span>Recent</span></button> <button type=\"button\" class=\"sidebar-nav-item\" :class=\"{ 'active': activeTab === 'settings' }\" @click=\"setTab('settings')\"><span class=\"sidebar-nav-icon\" aria-hidden=\"true\">⚙</span> <span>Settings</span></button></nav></aside><div class=\"main-area\"><header class=\"top-bar\"><div id=\"health-bar\" class=\"health-bar-inline\"></div><div class=\"header-controls\"><span class=\"filter-pill\" x-show=\"selectedProject\" x-cloak x-text=\"projectLabel()\"></span> <select class=\"project-select\" x-model=\"selectedProject\" @change=\"applyProjectFilter()\" aria-label=\"Filter by project\"><option value=\"\">All Projects</option> ")
		if templ_7745c5c3_Err != nil {
		...
		for _, p := range projects {
			...
			templ_7745c5c3_Var2, templ_7745c5c3_Err = templ.JoinStringErrs(p.Path)
			if templ_7745c5c3_Err != nil {
