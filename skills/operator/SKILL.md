...
| **WAL throttle** | Large `index.db-wal` | Effective workers drop (64 MB → 8, 128 MB → 4, 256 MB → 2) while **target** stays high, then keep **halving every 30s — to 0 if needed** — while the WAL is not draining. Aux is capped the same way. At 0 the server forces a TRUNCATE once in-flight embeds finish; workers climb back as the WAL shrinks. UI: “WAL n/target”. See [SQLite WAL runbook](#sqlite-wal-runbook). |
